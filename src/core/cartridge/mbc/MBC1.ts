...
export class MBC1Mapper implements Mapper {
  ...
  private lowBankBits = 1;

  private highBankBits = 0;
  ...
  public readRom(address: number): number {
    if (address < 0x4000) {
      const bank = this.mode === 0 ? 0 : this.getUpperBankBits() << 5;
      const bankOffset = (bank % this.romBanks) * 0x4000;
      return this.rom[bankOffset + address] ?? 0xff;
    }

    const bank = this.getSwitchableBank();
  ...
  public writeRom(address: number, value: number): void {
    ...
    if (address <= 0x3fff) {
      const low = maskedValue & 0x1f;
      this.lowBankBits = low === 0 ? 1 : low;
      return;
    }

    if (address <= 0x5fff) {
      this.highBankBits = maskedValue & 0x03;
      return;
    }

    this.mode = maskedValue & 0x01;
  }
  ...
  private getUpperBankBits(): number {
    return this.highBankBits & 0x03;
  }

  private getSwitchableBank(): number {
    let bank = this.lowBankBits | (this.getUpperBankBits() << 5);

    bank %= this.romBanks;
    if ((bank & 0x1f) === 0) {
      bank = (bank + 1) % this.romBanks;
      if (bank === 0) {
        bank = 1;
      }
    }
  ...
  private getRamBank(): number {
    if (this.mode === 0 || this.ramBanks <= 1) {
      return 0;
    }

    return (this.highBankBits & 0x03) % this.ramBanks;
  }
}
